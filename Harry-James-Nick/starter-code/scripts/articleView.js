...
articleView.populateFilters = function () {
  $('article').each(function () {
    ...
    if (!$(this).hasClass('template')) {
      ...
      // TODONE?: HarryH - Refactor this concatenation using a template literal.
      // optionTag = '<option value="' + authorName + '">' + authorName + '</option>';
      // the ` is derived from es6
      optionTag = `<option value="${authorName}">${authorName}</option>`;
      ...
      // again, changing from above to below using template literal ${} and `
      optionTag = `<option value="${category}">${category}</option>`;
...
articleView.handleAuthorFilter = function () {
  $('#author-filter').on('change', function () {
    ...
    if ($(this).val()) {
      // TODONE?: HarryH - If the <select> menu was changed to an option that has a value, we first need to hide all the articles, and then show just the ones that match for the author that was selected.
      // Use an "attribute selector" to find those articles, and fade them in for the reader.

      $('#author').hide();
      $('#author[data-author="' + $(this).val()) + '"]"'.show().fadeIn('slow');


    } else {
      // TODONE?: HarryH - If the <select> menu was changed to an option that is blank, we should first show all the articles, except the one article we are using as a template.
      $('article').show();
      $('.template').hide();
    }
    $('#category-filter').val('');
...
articleView.handleCategoryFilter = function () {
  // TODONE?: HarryH - Just like we do for #author-filter above, we should handle change events on the #category-filter element.
...
articleView.handleMainNav = function () {
  // TODONE?:HarryH - Add an event handler to nav elements that will power the Tabs feature.
...
articleView.setTeasers = function () {
  ...
  // TODONE?: HarryH - Add an event handler to reveal all the hidden elements, when the .read-on link is clicked. You can go ahead and hide the "Read On" link once it has been clicked. Be sure to prevent the default link-click action!
...
// TODONE?: HarryH - Call all of the above functions, once we are sure the DOM is ready.
