...
articleView.handleMainNav = function() {
  ...
  $('nav ul').on('click', function(e) {
    ...
    if (myTarget.trim().toLowerCase() === 'home') {
      $('#articles').show();
    }
    else if (myTarget.trim().toLowerCase() === 'about') {
      $('#about').show();
    }
  });
}
...
articleView.setTeasers = function() {
  ...
  $('#articles').on('click', function (e) {
    ...
    const $myArticleBody  = $myTarget.prev();
    $($myArticleBody).find('p').css('display','block');
    const myButton = $myArticleBody.nextAll('a');
    myButton.hide();
  });
}
