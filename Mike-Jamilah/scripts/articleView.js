...
articleView.handleAuthorFilter = function() {
  $('#author-filter').on('change', function() {
    ...
    if ($(this).val()) {
      ...
      $('article').hide();
      $(`article[data-author="${$(this).val()}"]`).fadeIn('400');
    } else {
    ...
    $('#category-filter').val('');
      ...
      // TODO: If the <select> menu was changed to an option that is blank, we should first show all the articles, except the one article we are 
      //using as a template.
...
articleView.handleCategoryFilter = function() {
  $('#category-filter').on('change', function() {
    if ($(this).val()) {
      ...
      $('article').hide();
      $(`article[data-category="${$(this).val()}"]`).fadeIn('400');
...
articleView.setTeasers = function() {
  ...
  $('#articles').delegate('.read-on', 'click', function(){
    event.preventDefault();
    $(this).hide();
    $(this).prev('.article-body').contents().show();
    $(this).prev('.article-body').parent('article').append(`<a href="#" class="show-less">Show less &larr;</a>`);
  });
};
